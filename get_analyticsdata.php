...
while($r = mysql_fetch_assoc($result)) {
   ...
   if(!isset($google_JSON)){    
     $google_JSON = "google.visualization.Query.setResponse({\"version\":\"0.6\",\"reqId\":".$reqID.",\"status\":\"ok\",\"sig\":\"1029305521\",\"table\": { \"cols\": [";
     $column = array_keys($r);
     
		 $modeID = $_GET['modeID'];
		 if($modeID=='daily')
		 {
		 $google_JSON_cols[]="{ \"id\": \"1\", \"label\": \"Time\", \"type\": \"number\"}, { \"id\": \"0\", \"label\": \"Demand (Kw-H)\", \"type\": \"number\"}";
		 }
		 else{
		 $google_JSON_cols[]="{ \"id\": \"1\", \"label\": \"Date\", \"type\": \"date\"}, { \"id\": \"0\", \"label\": \"Demand (Kw-H)\", \"type\": \"number\"}";
		 }	 
		 ...
		 $google_JSON_rows[] = "{ \"c\":[{ \"v\": [".$time."]}, { \"v\": ".$r['usage']."}]}";
		 }
		 else{
		 $date = $r['start_date'];
		 $date = formatDate($date);
		 $google_JSON_rows[] = "{ \"c\":[{ \"v\": new Date(".$date.")}, { \"v\": ".$r['usage']."}]}";
		 }
}    
